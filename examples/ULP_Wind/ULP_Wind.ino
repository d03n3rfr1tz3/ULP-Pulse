...
#define WINDFACTOR        2.4         // 2.4 km/h per pulse defined by the Anemometer
...
void windTest() {
  ...
  if (timeSleep > 0) {
    ...
    if (pulsesRaw >= 0) {
      ...
      wind = ((float)pulsesRaw / (float)timeSleep) * WINDFACTOR;
      windSuccess = wind < 27.5F;
    }
    ...
    if (pulsesRaw > 1 && shortestPulseRaw > ULPSLEEP * 5 && shortestPulseRaw < timeSleep * TIMEFACTOR) {
      ...
      windGust = (1 / ((float)shortestPulseRaw / (float)TIMEFACTOR)) * WINDFACTOR;
      windGustSuccess = windGust < 27.5F;
    }
  }

  Serial.print(F("Wind Speed: "));
  if (windSuccess) Serial.print(wind);
  Serial.print(F("km/h | "));
  if (windSuccess) Serial.print(wind / 3.6F);
  Serial.println(F("m/s"));
  
  Serial.print(F("Wind Gust Speed: "));
  if (windGustSuccess) Serial.print(windGust);
  Serial.print(F("km/h | "));
  if (windGustSuccess) Serial.print(windGust / 3.6F);
  Serial.println(F("m/s"));
